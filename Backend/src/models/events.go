...
type Event struct {
	...
	TimeFrameID              uint            `gorm:"not null"`               // The time frame of the event.
	...
	OrganisationID           uint            `gorm:"not null"` // The organisation hosting the event (foreign key reference to Organisations.Id).
	RoomReservation          RoomReservation // Referenced RoomReservation Object
	RoomReservationID        uint            `gorm:"not null"` // The room reservation for the event (foreign key reference to RoomReservations.Id).
	ItemReservation          ItemReservation // Referenced ItemReservation Object
	ItemReservationID        uint            `gorm:"not null"`     // The item reservation for the event (foreign key reference to ItemReservations.Id).
	...
	CreatedByID              uint            `gorm:"not null"` // The user who created the event (foreign key reference to Users.Id).
}
